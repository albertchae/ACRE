...
Note that the highest level of reproducibility, which requires complete reproducibility starting from  raw data, is very demanding and should not be expected of all published research (especially before the updated AEA policy in 2019). Instead, this standard is set as an aspiration to improve the current reproducibility of research and facilitate the transmission of knowledge in the scientific community.
...
2.	[**Assessment**](#assessment), where you will review and describe in detail the available reproduction package (or the “reproduction materials”) and provide an assessment of the current level of computational reproducibility of the selected outputs; 
...
Figure 2 depicts suggested levels of effort for each stage of the exercise depending on the context in which you perform the reproduction. This process need not be chronologically linear, e.g., you may realize that the scope of a reproduction is too ambitious and switch to a less intensive reproduction. Later in the exercise, you can also begin testing different specifications for robustness while also assessing a paper's level of reproducibility.

## Recording the results of the exercise {-}

You will be asked to record the results of their reproduction progress through each stage. As part of Stage 1:[Scoping](#scoping), you will complete **[Survey 1](https://berkeley.qualtrics.com/jfe/form/SV_8hLHNI6LGSYchEN)**. This step may also involve writing a brief 1-2 page summary of the paper (check this with your supervisor). For Stage 2: [Assessment](#assessment) and Stage 3: [Improvement](#improvements), you will complete **Survey 2** [add link]. As part of Stage 2, you will also complete **[this mapping tool](https://docs.google.com/spreadsheets/d/1Uj5rEwSpFh_RXsmRhFnbz8cL88PUA5cRKp_38xV4eeE/copy?usp=sharing)** to record each item found in the paper's reproduction package (raw data, analysis data, and scripts) and connect them to the output that they produce (a table, graph, estimate, etc.). As part of Stage 4: [Robustness Checks](#robust) you will use **Survey 3** [add link].
